...
FloraFont *add_flora_font(FloraApplicationState *state, const char *path, const float point_size, char *font_name)
{
	if (!state) {
	...
	void *existing = NULL;
	if (get_table(&state->font_table, &existing, font_name)) {
	...
	if (!set_table(&state->font_table, flora_font, font_name)) {
		TTF_CloseFont(font);
		free(flora_font);
